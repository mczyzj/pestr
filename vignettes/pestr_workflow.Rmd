...
In order to start working with **pestr** package, you should register yourself (*free of charge*) to [EPPO Data Services](https://data.eppo.int/). Then run `create_eppo_token()` which will store your credentials in variable used by functions that connect to **REST API**.

```{r eval = FALSE}
create_eppo_token('<<your_EPPO_token>>')
```

Next, you can run `eppo_database_download()` function that will download (by default to your **working directory**, which can be override with `filepath` argument) archive with the *SQLite* file. If you are on **Linux** operating system, file will be extract it into your working directory. On **Windows** you will be asked to extract the database file manually.
...
We can now parse `virs_eppocodes` as `raw_eppocodes` argument, and in consequence receive taxonomy of viroids and viruses only. 
```{r eval = FALSE}
...
`eppo_tabletools_distri()` does not connect to REST API, but it downloads information from \*.csv files directly from **EPPO Global Database**. As a consequence there is no `token` argument (since it does not need the EPPO token) -- a variabale containing result of `eppo_names_tables`.
