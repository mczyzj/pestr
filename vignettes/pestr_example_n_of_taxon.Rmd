...
Obtaining number of pests taxons is not very complicated. Since we can use `eppocodes` directly, we can parse whole column to `eppo_tabletools_taxo` function, which will retrieve data on pests taxonomy.
...
Now, we can parse `pest_eppocodes` variable as `raw_eppocodes` arugument:
